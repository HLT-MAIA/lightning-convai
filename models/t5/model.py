...
=============================
    T5 Sequence-to-Sequence Model implementing the PyTorch Lightning interface that can be used to train a taskmaster Chatbot.
"""
...
class AssistantT5(AssistantBase):
    ...
    def __init__(self, hparams: Namespace):
        ...
        self.hparams.model = "AssistantT5"
        ...
        self.t5.resize_token_embeddings(self.tokenizer.vocab_size)
        self.t5.config.update(
            {
        ...
        self.multiple_choice_head = SequenceSummary(self.t5.config)
    ...
    def forward(
        ...
        decoder_hidden_states = t5_output.decoder_hidden_states[-1]
        batch_sz, candidates, seq_sz = decoder_input_sz
        hidden_states = decoder_hidden_states.view(
            batch_sz, candidates, seq_sz, self.t5.model_dim
        )
        ...
        return AssitantModelOutput(
            lm_loss=t5_output.loss,
            mc_loss=mc_loss,
            lm_logits=t5_output.logits,
            mc_logits=mc_logits,
            past_key_values=t5_output.past_key_values,
            hidden_states=hidden_states,
            attentions=t5_output.decoder_attentions,
        )
